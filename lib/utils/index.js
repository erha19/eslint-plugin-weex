...
module.exports = {
  ...
  defineTemplateBodyVisitor (context, templateBodyVisitor, scriptVisitor) {
    if (context.parserServices.defineTemplateBodyVisitor == null) {
      context.report({
        loc: { line: 1, column: 0 },
        message: 'Use the latest vue-eslint-parser. See also https://github.com/vuejs/eslint-plugin-weex#what-is-the-use-the-latest-vue-eslint-parser-error'
      })
      return {}
    }
  ...
  * iterateFunctionExpression (node, groupName) {
  ...
  getStyleContent (text) {
    const content = /<style\s?(scoped)?.*>([\s\S]*?)<\/style>/gi.exec(text)
  ...
  checkStyleRoot (text) {
    const hasStyle = /\s*?<style\s*?.*>/i.exec(text)
    const hasScoped = /<style\s*?.*scoped\s*?.*>([\s\S]*?)<\/style>/i.test(text)
    const line = text.slice(0, hasStyle.index).split('\n').length + 1
    const leftWord = text.slice(hasStyle.index + 1).split('\n')[0]
    const column = text.slice(hasStyle.index + 1, hasStyle.index + leftWord.length + 1)
    let startColumn = /\S+/.exec(column)
    let endColumn = /\s*?$/.exec(column)
    if (endColumn) {
      endColumn = endColumn.index - 1
    } else {
    ...
    if (hasStyle && !hasScoped) {
      const loc = {
        start: { line: line, column: startColumn },
