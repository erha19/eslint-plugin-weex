...
module.exports = {
  ...
  * iterateFunctionExpression (node, groupName) {
  ...
  checkStyleRoot (text) {
    const hasStyle = /.*?<style\s*?.*>/i.exec(text)
    ...
    let startColumn = /\S+/.exec(leftWord)
    let endColumn = /\s*?$/.exec(leftWord)
    if (endColumn.index) {
      endColumn = endColumn.index
    } else {
      endColumn = leftWord.length
    }
    if (startColumn) {
      startColumn = startColumn.index + 1
    } else {
      startColumn = 0
    }
