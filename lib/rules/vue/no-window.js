...
module.exports = {
  meta: {
    docs: {
      description: 'disallow window api is invalid in weex',
      category: 'essential',
      url: 'https://github.com/weexteam/eslint-plugin-weex/blob/v1.1.8/docs/rules/no-window.md'
    },
    fixable: null,
    schema: []
  },
  ...
  create (context) {
      ...
      if (nodes && nodes.length > 0) {
        nodes.forEach(objectExpressionNode => {
          ...
          if (objectExpressionNode.value.type === 'FunctionExpression') {
            functionExpressionNode = functionExpressionNode.concat(objectExpressionNode.value)
    ...
    return utils.executeOnVue(context, obj => {
      ...
      const propsWithFunctionExpression = findPropsWithFunctionExpression(functionExpression, 'window')
      propsWithFunctionExpression.forEach(prop => {
        context.report({
          node: prop,
          loc: prop.loc,
          message: 'window api is invalid in weex.'
        })
      })
    })
  }
