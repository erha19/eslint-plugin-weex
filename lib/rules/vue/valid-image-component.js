...
module.exports = {
  meta: {
    docs: {
      ...
      category: 'essential',
      url: 'https://github.com/weexteam/eslint-plugin-weex/blob/v1.0.3/docs/rules/valid-image-component.md'
    },
    fixable: null,
    schema: []
  },
        ...
        attrs.forEach(attr => {
          if (attr.key.type === 'VIdentifier') {
            ...
            if (attr.key.name === 'bind' && attr.key.argument === 'style') {
              let expressions = attr.value.expression.properties
            ...
            if (cssTreeNode.type === 'StyleSheet' && cssTreeNode.children.head) {
              ...
              while (current) {
                ...
                while (preludeCurrent) {
                  if (classs.indexOf(preludeCurrent.data.children.head.data.name) > -1) {
                    if (datas.prelude) {
