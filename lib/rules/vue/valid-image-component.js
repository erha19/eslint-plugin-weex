...
module.exports = {
  meta: {
    docs: {
      ...
      category: 'essential',
      url: 'https://github.com/weexteam/eslint-plugin-weex/blob/v1.1.5/docs/rules/valid-image-component.md'
    },
    fixable: null,
    schema: []
  },
  ...
  create (context) {
    ...
    return utils.defineTemplateBodyVisitor(context, {
      "VElement[name='image']" (node) {
        ...
        if (style) {
          const keys = Object.keys(style).map(key => {
        ...
        if (classs && Array.isArray(classs)) {
          cssTree.walk(ast, function (cssTreeNode) {
            if (cssTreeNode.type === 'StyleSheet' && cssTreeNode.children && cssTreeNode.children.head) {
              ...
              while (current) {
                ...
                let preludeCurrent = datas.prelude && datas.prelude.children && datas.prelude.children.head
